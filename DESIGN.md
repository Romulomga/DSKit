...
colors-dark:
  ...
  background: "#121212"
  background-grouped: "#0A0A0A"
  surface: "#1E1E1E"
  surface-elevated: "#2A2A2A"
  on-surface-high: "#EDEDED"
  on-surface-medium: "#B0B0B0"
  on-surface-light: "#4A4A4A"
  border: "#363636"
  hairline: "#FFFFFF @ 10%"
  disabled: "#A4A4A4"
  on-disabled: "#707070"
...
**`settings-row`** — `DSSettingsRow`. Padding de 12pt. Tile de ícone opcional de 28pt (`{rounded.sm}`, `{colors.accent}` por padrão ou uma cor do chamador, glifo branco 15pt/600), título `{typography.body}` `{colors.on-surface-high}` (ou `{typography.headline}` com `titleStyle: .prominent`, para entradas que levam a conteúdo, como categorias de ajuda; nesse estilo o padding vertical sobe para `{spacing.lg}`), subtítulo opcional `{typography.footnote}` secondary com 2pt de espaço, slot trailing (toggle, valor), e um chevron tertiary 13pt/600 só quando existe uma ação. Divisor `{colors.border}` de 0.5pt com recuo de 52pt da borda esquerda; passe `showsDivider: false` na última row. Rows com ação viram um `Button` plain com haptic light; rows sem ação continuam como container passivo para que o controle trailing continue vivo.
...
**`paged-tabs`** — `DSPagedTabs`. Abas com páginas que deslizam na horizontal (paging nativo), faixa de rótulos em cima: rótulo `{typography.subheadline}` 400 secondary, 600 e `{colors.accent}` quando ativo, altura mínima 44; indicador em cápsula `{colors.accent}` de 3 pt cuja posição e largura seguem o deslize (contínuo no iOS 18, por seleção no 17); hairline sob a faixa; até três seções dividem a largura, mais rolam. Slot `header` entre a faixa e as páginas para busca/filtros. Estilo `.segmented`: a faixa é a pílula do `segmented-picker` `.filled` (trilho `{colors.surface}` com hairline, cápsula `{colors.accent}` de 36 pt, rótulo `{typography.footnote}` 600 branco sobre ela), com a cápsula posicionada pelo progresso do deslize; `stripInset` controla a margem lateral. VoiceOver ouve o nome da aba ao trocar; páginas fora da tela ficam ocultas.
